...
public class Queue<T> {
    ...
    /**
     * Desencola un elemento de la cola por su indice
     * @param idx
     * @param mainMemory
     * @return 
     */
        ...
        if(!isEmpty()){
            if(idx == 0){
                ...
                for (int i = 0; i < this.size+1; i++) {
                    Integer memoryAdressAux = (Integer) dequeue();
                    if(i != idx){
                        enqueue((T) memoryAdressAux);
                    }                   
                    else{
                        memoryAdress = memoryAdressAux;
                    }
                }
                return memoryAdress;                                              
            }
    ...
    public Integer dequeueSPN(MemoryEntity[] mainMemory){
        Integer idx = shortestProcessIndexSPN(mainMemory);                        
        return dequeueProcessByIndex(idx, mainMemory);                
    }
    
    public Integer dequeueSRT(MemoryEntity[] mainMemory){
        Integer idx = shortestProcessIndexSRT(mainMemory);                
        return dequeueProcessByIndex(idx, mainMemory);                
    }
    
    public Integer dequeueHRRN(MemoryEntity[] mainMemory){
